...
layoutOriginal = [
    ...
    [sg.Frame(layout=[      
    [sg.Radio('CSV - rescue', "RADIO1", key="CSV", default=False, size=(20,1)), sg.Radio('Distribution analysis', "RADIO1", key="Distribution", default=False, size=(20,1)), sg.Radio('Correlation analysis', "RADIO1", key="Correlation",  default=False, size=(20,1))],         
    [sg.Radio('Regression analysis', "RADIO1", key="Regression", default=False, size=(20,1)), sg.Radio('Mediation analysis', "RADIO1", key="Mediation", default=False, size=(20,1)), sg.Radio('Moderation analysis', "RADIO1", key="Moderation", default=False, size=(20,1))],
    [sg.Radio('Factor analysis', "RADIO1", key="Factor", default=False, size=(20,1)), sg.Radio('Edit dataset', "RADIO1", key="Compute", default=False, size=(20,1))]], title='Options', title_color='red', relief=sg.RELIEF_SUNKEN, tooltip='Use these to set flags')],    
    [sg.Text('')],
...
columnRight = [
    ...
    [sg.Text('"Regression analysis" outputs simple, or multiple, regression values and VIF values. Plots and tables may be exported (optional).', size=(45,3), background_color='lightsteelblue2')],
    ...
    [sg.Text('"Factor analysis" performs exploratory analysis, confirmatory analyses and other factor analyses. Values may be exported (optional).', size=(45,3), background_color='lightsteelblue2')],  
    ...
    [sg.Text('For all analyses except "CSV - rescue":', font=('bold'), background_color='lightsteelblue2')],
    [sg.Text('Choose dataset (NB: Dataset must be in ".xlsx" or ".csv". All variables to be analysed should be numeric.):', size = (45, 2), background_color='lightsteelblue2')],
    [sg.In('', key='analytics_dataset', size=(43,1)), sg.FileBrowse()],
    [sg.Text('', background_color='lightsteelblue2')]]

layoutRight = [
    [sg.Frame('Description', columnRight, size=(415,615), background_color='lightsteelblue2')]]  
...
layoutComplete = [
    [sg.Column(layoutOriginalFrame, element_justification='left', size=(580,620)), sg.Column(layoutRight, element_justification='right', size=(495,620))], 
    ...
    while True:
                        ...
                        while True:
                            ...
                            if valDistribution['distribution_output_yes']==True:
                                ...
                                if valDistribution['plots']==True:
                                    ...
                                    print('Pairwise plots for ' + varnames + ' saved')
                                    plt.close(fig)
                                
                                if valDistribution['values']==True:
                                    engine = 'xlsxwriter'
                                    with pd.ExcelWriter(os.path.join(output_folder, 'Values_'+varnames+'.xlsx')) as writer:                     
        ...
        if (not winCorrelation_active) and (valOriginal['Correlation']==True) and (evOriginal=='Continue') and ((data_in.endswith('.csv')) or (data_in.endswith('.xlsx'))):
            ...
            layoutCorrelation = [
                ...
                [sg.Text('Divide variables by commas.')],
        ...
        if (not winRegression_active) and (valOriginal['Regression']==True) and (evOriginal=='Continue') and ((data_in.endswith('.csv')) or (data_in.endswith('.xlsx'))):
            ...
            layoutRegression = [
                ...
                [sg.Text('For multiple regression, separate IVs by commas.')],
                [sg.InputText('', key='iv')],
                [sg.Text('')],
            ...
            while True:
                ...
                #Valdidate variables supplied
                    ...
                    #create variables string for ols
                    var_str=''
                    len_list=len(varsplit)
                    i=1
                    for var in varsplit:
                        if i == 1:
                            var_str=var + ' ~ '
                        elif i > 1 and i <= len_list:
                            var_str=var_str + ' + ' + var
                        i += 1
                            ...
                            if regression_type=='multiple':
                                ...
                                if valRegression['regression_output_yes']==True:
                                    ...
                                    if valRegression['regression_plots']==True: #Make and save ccpr plots:
                                        ...
                                        plt.savefig(os.path.join(output_folder, 'Multiple_OLS_')+iv+'_'+dv+'_regression_plots.png', dpi=300, format='png', transparent=True)
                                        plt.savefig(os.path.join(output_folder, 'Multiple_OLS_')+iv+'_'+dv+'_regression_plots.pdf', dpi=300, format='pdf')
                                        plt.close(fig)
        ...
        if not winMediation_active and valOriginal['Mediation']==True and evOriginal=='Continue' and ((data_in.endswith('.csv')) or (data_in.endswith('.xlsx'))):
            ...
            layoutOriginalFrame = [
                ...
                [sg.Text('Enter mediating variable (single, or multiple parallel):', font=('bold'))],
                [sg.InputText('', key='m', size=(20,1))],
                [sg.Text('Enter names of covariates (optional):', font=('bold'))],
                    ...
                    while True:
                        ...
                        if valFactor['kai_crit']==True:
                            ...
                            for value in list_ev:
                                if float(value)>1:
                                    nr_eigenvalues_above_1.append(value)
                            nr=len(nr_eigenvalues_above_1)
                            eigenvalues_df=pd.DataFrame(list_ev, index=fact_variables)
        ...
        if (not winDocumentation_active) and (evOriginal=='Documentation'):
            ...
            layoutDoc2 = [
                ...
                [sg.Text("Factor-analyzer 0.4.0 Matplotlib 3.5.1.", background_color='lightsteelblue2')],
                [sg.Text("", background_color='lightsteelblue2')]]
